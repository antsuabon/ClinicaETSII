...
public interface SpringDataPrescriptionRepository extends PrescriptionRepository, CrudRepository<Prescription, Integer> {
	...
	@Override
	@Query("SELECT DISTINCT prescription FROM Prescription prescription WHERE prescription.patient.id =:id")
	Collection<Prescription> findPrescriptionByPatientId(@Param("id") int patientId);
