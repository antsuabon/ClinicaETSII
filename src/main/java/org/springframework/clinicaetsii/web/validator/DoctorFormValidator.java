...
public class DoctorFormValidator implements Validator {
	...
	public void validate(final Object target, final Errors errors) {
		...
		if (doctor.getDni() == null || StringUtils.isEmpty(doctor.getDni())) {
			...
			errors.rejectValue("doctor.dni", "invalidDniPattern", "El campo debe seguir el formato: 12345678A");
		}
		...
		if (doctor.getEmail() == null || StringUtils.isEmpty(doctor.getEmail())) {
			...
			errors.rejectValue("doctor.email", "invalidEmailPattern", "No se ha introducido un Email adecuado");
		}
		...
		if (doctor.getPhone() == null || StringUtils.isEmpty(doctor.getPhone())) {
			...
			errors.rejectValue("doctor.phone", "invalidPhonePattern", "No se ha introducido un número de teléfono adecuado");
		}

		if (doctor.getCollegiateCode() == null || StringUtils.isEmpty(doctor.getCollegiateCode())) {
			errors.rejectValue("doctor.collegiateCode", "requiredCollegiateCode", "Este campo es obligatorio");
		} else if (!this.collegiateCodePattern.matcher(doctor.getCollegiateCode()).matches()) {
			errors.rejectValue("doctor.collegiateCode", "invalidCollegiateCodePattern", "Este campo debe de estar formado por 9 dígitos");
		} else {
			if (Integer.valueOf(doctor.getCollegiateCode().substring(0, 2)).equals(0) || Integer.valueOf(doctor.getCollegiateCode().substring(0, 2)).compareTo(52) > 0) {
				errors.rejectValue("doctor.collegiateCode", "invalidCollegiateCodePatternFirstPair", "La primera pareja de dijitos es erronea");
			}
			...
			if (Integer.valueOf(doctor.getCollegiateCode().substring(2, 4)).equals(0) || Integer.valueOf(doctor.getCollegiateCode().substring(2, 4)).compareTo(52) > 0) {
				errors.rejectValue("doctor.collegiateCode", "invalidCollegiateCodePatternSecondPair", "La segunda pareja de dijitos es erronea");
			}
		}
		...
		if (doctorForm.getNewPassword() != null && !StringUtils.isEmpty(doctorForm.getNewPassword())) {
			...
			if (!doctorForm.getNewPassword().equals(doctorForm.getRepeatPassword())) {
				errors.rejectValue("repeatPassword", "unrepeatedPassword", "La contraseña no coincide");
			}
		}
		...
		if (!oldDoctor.getUsername().equals(doctor.getUsername())) {
			if (this.doctorService.findDoctorByUsername(doctor.getUsername()) != null) {
				errors.rejectValue("doctor.username", "alreadyExistingUsername", "Ya existe un usuario con este nombre de usuario");
			}
		}
